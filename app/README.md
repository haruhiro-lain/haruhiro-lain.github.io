...
├── app/                    # 代码目录（通常无需修改）
│   ├── public/
│   ├── scripts/            # 同步脚本（数据写入 app 外的 data/、cache/）
│   ├── src/
│   │   ├── assets/
│   │   ├── components/
│   │   ├── composables/
...
│   ├── package.json
│   └── tsconfig.json
├── content/                # 📝 博文（md/mdx）— 日常更新
│   ├── learning/
│   └── life/
├── data/                   # 📊 站点数据（steamRecentGames、gkModelSelection 等）
├── TO-DO/                  # 📋 任务清单草稿
├── cache/                  # 🖼 图片缓存（steam 封面、GitHub 头像等）
├── docker/                 # 🐳 Docker 部署配置（Dockerfile、compose、nginx）
├── build.ps1               # Docker 构建部署入口
├── dev.ps1                 # 本地开发入口
└── .dockerignore
```

> 日常更新博客只需编辑根目录 `content/`、`data/`、`TO-DO/`、`cache/` 下的文件，无需改动 `app/` 内代码。
...
- `content/`（app 外）存放内容文件（`md/mdx`），schema 定义在 `src/content.config.ts`。
- 页面路由统一放在 `src/pages`。
